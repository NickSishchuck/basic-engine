...
OpenGLRendererWrapper::OpenGLRendererWrapper()
    : window(nullptr), floorEnabled(true), floorSize(20.0f), gridLineCount(20),
      autoUpdateFloor(true), lastFloorSize(20.0f), lastGridLineCount(20),
      vertices(nullptr), indices(nullptr), verticesSize(0), indicesSize(0),
      windowWidth(800), windowHeight(600), viewportWidth(800),
      viewportHeight(600), isRenderingToViewport(false) {

  std::cout << "DEBUG: OpenGLRendererWrapper constructor called" << std::endl;

  // Triangle vertices
  verticesSize = 30 * sizeof(GLfloat);
  ...
  indicesSize = 18 * sizeof(GLuint);
  indices = new unsigned int[18]{0, 1, 2, 0, 2, 3, 0, 1, 4,
                                 1, 2, 4, 2, 3, 4, 3, 0, 4};

  std::cout << "DEBUG: OpenGLRendererWrapper constructor completed"
            << std::endl;
}
...
bool OpenGLRendererWrapper::Initialize(int width, int height,
                                       const char *title) {
  ...
  try {
    viewportFramebuffer =
        std::make_unique<Framebuffer>(viewportWidth, viewportHeight);
  ...
  try {
    shader = std::make_unique<Shader>("shaders/default.vert",
                                      "shaders/default.frag");
  } catch (const std::exception &e) {
    std::cerr << "Failed to load shaders: " << e.what() << std::endl;
    return false;
  } catch (int errnum) {
    std::cerr << "Failed to load shaders - errno: " << errnum << " - "
              << strerror(errnum) << std::endl;
    std::cerr << "Make sure shader files exist at: shaders/default.vert and "
                 "shaders/default.frag"
              << std::endl;
    return false;
  }
  ...
  vao->LinkAttrib(*vbo.get(), 1, 3, GL_FLOAT, 6 * sizeof(float),
                  (void *)(3 * sizeof(float)));
  ...
  camera =
      std::make_unique<Camera>(width, height, glm::vec3(0.0f, 3.0f, 10.0f));
...
void OpenGLRendererWrapper::BeginFrame() {
  ...
  if (floorEnabled) {
    ...
    if (ImGui::IsItemHovered()) {
      ImGui::SetTooltip(
          "When enabled, floor updates instantly as you drag sliders");
    }
...
void OpenGLRendererWrapper::CreateCube() {
  ...
  // Cube indices
  static GLuint cubeIndices[] = {// Front face
                                 0, 1, 2, 2, 3, 0,
                                 // Right face
                                 1, 5, 6, 6, 2, 1,
                                 // Back face
  ...
  cubeVAO->LinkAttrib(*cubeVBO.get(), 0, 3, GL_FLOAT, 6 * sizeof(float),
                      (void *)0);
  cubeVAO->LinkAttrib(*cubeVBO.get(), 1, 3, GL_FLOAT, 6 * sizeof(float),
                      (void *)(3 * sizeof(float)));
...
void OpenGLRendererWrapper::RenderCube(const glm::vec3 &position,
...
void OpenGLRendererWrapper::CreateFloorPlane(float size) {
  ...
  floorVAO->LinkAttrib(*floorVBO.get(), 0, 3, GL_FLOAT, 6 * sizeof(float),
                       (void *)0);
  floorVAO->LinkAttrib(*floorVBO.get(), 1, 3, GL_FLOAT, 6 * sizeof(float),
                       (void *)(3 * sizeof(float)));
...
void OpenGLRendererWrapper::CreateGridLines(float size, int gridLines) {
  ...
  gridVBO = std::make_unique<VBO>(gridVertices.data(),
                                  gridVertices.size() * sizeof(GLfloat));

  gridVAO->LinkAttrib(*gridVBO.get(), 0, 3, GL_FLOAT, 6 * sizeof(float),
                      (void *)0);
  gridVAO->LinkAttrib(*gridVBO.get(), 1, 3, GL_FLOAT, 6 * sizeof(float),
                      (void *)(3 * sizeof(float)));
...
void OpenGLRendererWrapper::RenderFloor(float size, int gridLines) {
  ...
  glDrawArrays(
      GL_LINES, 0,
      (gridLines + 1) *
          4); // 2 points per line, (gridLines+1) lines in each direction
  gridVAO->Unbind();
}
